...
int scrutation(int socket_ecoute) {
	...
	while (1) {
		...
		int nbSocketReady = select(nfds+1, &readfs, NULL, NULL, NULL);
		...
		for (int i = 0; i < MAXCLIENT; ++i) {
			if (tableau[i]->socket != -1) {
				if(FD_ISSET(tableau[i]->socket, &readfs)) {
					if (redirige_trafic_sur_stdout(tableau[i]) <= 0) {
		...
		if (FD_ISSET(socket_ecoute, &readfs)) {
			...
			TClient * new_client = ajoute_client(new_socket);
...
int launch_serv(char * port) {
	init_tableau();
	int socket_ecoute = cree_socket_ecoute(port);
